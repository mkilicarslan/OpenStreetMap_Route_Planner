...
float RoutePlanner::CalculateHValue(RouteModel::Node const *node) {
    float distance = node->distance(*RoutePlanner::end_node);
    return distance;
}
...
RouteModel::Node *RoutePlanner::NextNode() {
   	std::sort(open_list.begin(), open_list.end(), [](const auto &i1, const auto &i2) {
...
std::vector<RouteModel::Node> RoutePlanner::ConstructFinalPath(RouteModel::Node *current_node) {
    ...
    while (current_node->parent != nullptr) {
        RoutePlanner::distance += current_node->distance(*current_node->parent);
...
void RoutePlanner::AStarSearch() {
    ...
    while (open_list.size() > 0)  {
        ...
        if (current_node->distance(*end_node) == 0) {
            std::vector<RouteModel::Node> path = RoutePlanner::ConstructFinalPath(current_node);
            RoutePlanner::m_Model.path = path;
            return;
        } else {
            RoutePlanner::AddNeighbors(current_node);
        }
